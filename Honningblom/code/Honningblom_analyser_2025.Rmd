...
param.est.andel <- function(serie = NULL,
  ...
  # ForsC8ker C% reskalere rutestC8rrelsene for hvert C%r slik at
  # antall planter i hver analyserute er i samsvar med en
  # binomisk fordeling. Dette er nC8dvendig for enkelte C%r i TeneskjC&r
  # populasjonen der det er ekstremt mange planter i enkelte av analyse-
  # rutene, som medfC8rer at ingen aktuelle fordelingsmodeller (poisson,
  # ZIP, hurdeled poisson, binomisk, ZI neg.binomisk osv.) fungerer som en
  # modell for dataene.
  
  # Reskalering av rutene innebC&rer C% fordele de observerte
  # plantene i ei analyserute over sc.f reskalerte ruter og C8ke
...
estim.ua.rose <- function(x)
  ...
  # den beregner delen av det undersC8kte omrC%det ved lokaliteten som det ett
...
estim.pop.size <- function(transektruteserier = NULL,
  ...
  # Estimer populasjonsstC8rrelsene for hvert C%r som 
...
df_locality <- import("path/Herminium_monorchis_masterfile_v_2025.xlsx",sheet="Locality",na=c("NA"))

df_transect <- import("path/Herminium_monorchis_masterfile_v_2025.xlsx",sheet="Transect",na=c("NA"," "))

df_plot <- import("path/Herminium_monorchis_masterfile_v_2025.xlsx",sheet="Plot_herminium",na=c("NA"))
```
...
df_plot$Plot_ID_orig <- df_plot$Plot_ID_new # Tar vare pC% opprinnelig input
...
#rutedata$Rute_orig <- rutedata$Rute # Tar vare pC% opprinnelig input
#rutedata$Rute <- rutedata$Rute.ID # Kopierer variabler for C% unngC% rekoding senere i skriptet
...
grense <- 0.05 # NC%r andelen transektruter med Honningblom er mindre enn grense,
# gjennomfC8res parametrisk estimering av forekomstarealet. Kan 
# settes lavere nC%r en har mange transektruter, og individuelt 
# for hver populasjon. Men da mC% scriptet endres tilsvarende
# pC% linje 365 og 426.
pop <- list() # Resultat-objekt

for (loc in localities) {
  
  # Organiser transektdatasettet i ei transektrute x C%rMedTransektdata matrise
  ...
  transektbredde <- df_locality[df_locality$Locality==loc,"Transect_width"] [1]#Hvis dette ikke heter "Transektbredde" i datafilen, forC%rsaker det en feil. Usikker hvor det da mC% endres til "Transect_width" ogsC%?
  ...
  # Organiser analysedatasettet i ei analyserute x C%rMedAnalyser matrise
  # Tar i utgangspunktet med alle analyserutene fra lokaliteten, fjerner
  # deretter rutene som aldri i lC8pet av tidsserien inngC%r i forekomstarealet
  # og C%r uten data. Denne prosedyren kan virke unC8dvendig, men er trolig
  # nC8dvendig for C% inkludere analyserutene som i starten var utenfor 
  # forekomstarealet, men som senere blir en del av det.
  
  analyse.rute.data <- df_plot[df_plot$Locality == loc,]
  
  ####################################################################################
  #### Her er roten til alt ondt i 2022! Koden nedenfor er en ad hoc tilpasning til fjorC%rets
  #### input data der ingen av variablene "Rute" og "Rute.ID" var komplette.
  #### Koden under fungerte bra i fjor, men ikke i C%r fordi variabelen Rute nC% ikke er oppgitt
  ...
  #### alle ruter har 0 individer C8ker betraktelig i forhold til i fjor. Dermed oppstC%r det
  #### problemer nC%r en under likelihood-estimeringen leter etter optima vha. funksjonen
  #### nlminb (linje 118).
  #### Koden pC% linjene 231 - 245 medfC8rer at ingen ruter mangler navn slik at koden pC%
  #### linjer 343 - 344 blir overflC8dig. Men jeg lar den likevel stC%.
...
#analyse.rute.data$Plot_ID_old[mangler.navn] <- analyse.rute.data$Plot_ID_new[mangler.navn]
  ...
  # For Skjellvik populasjonen gjC8res antagelsen at transekt-datasettet fra 2021 
  # ogsC% reflekterer forekomstarealet gjennom hele perioden 2014 - 2020. 
  # Denne antagelsen er ikke rimelig for TeneskjC&r. Her er det 
  # dessuten ikke mulig C% estimere et forekomstareal ut fra transektdatasettet.
  
  # For Skjellvik populasjonen estimeres forekomstarealet for C%rene 2014-2017 
  # og 2019 - 2020 fra transektdatasettet for 2021. I praksis, kopieres 
  # transektdataene for 2021 for disse 6 C%rene. For Filletassen, Skjellvik og 
  # Skipstadsand estimeres forekomstarealet som andel transektruter med 
  # honningblom * stC8rrelse av undersC8kt areal. StC8rrelse av undersC8kt areal er
  # i linje-designet ved Skjellvik, Skipstadsand og TeneskjC&r lik 
  # transektbredden * summerte transekt-lengder. I rose-designet settes 
  # stC8rrelsen av det undersC8kte arealet lik summen av sektorenes areal, der
  # kun de reelle sektorene telles med, dvs. de som har ei transektrute.
  
  # For TeneskjC&r ser det ut til at forekomstarealet er blitt redusert i lC8pet
  ...
  # datasett fra hvert C%r, dvs. analysedata fra det aktuelle C%ret samt
  # transektdataene, basert pC% en veid likelihood funksjon. Vektene i funksjonen
  # er antall observasjoner i C%rets analysedatasett og i transektdataene.
  # Forekomstarealet ved TeneskjC&r for et aktuelt C%r estimeres som 
  # fat * stC8rrelsen av det undersC8kte arealet: 
  
  # fat <- pnbinom(0,par[1],par[2],lower.tail = FALSE)
  
  # Dette vil medfC8re at det estimerte forekomstarealet vil gC% ned i lC8pet av 
  # perioden fordi antall individer i flere av analyserutene er redusert til 0.
  
  # For Skipstadsand-populasjonen benyttes samme tilnC&rming som for populasjonen
  # pC% TeneskjC&r. Ved skipstadsand er det kun to forekomstruter i transektene,
  # noe som gir svC&rt usikre estimat av forekomstarealet. En parametrisk
  # tilnC&rming reduserer usikkerheten noe sammenliknet med en ikke-parametrisk
  # bootstrap tilnC&rming, naturlig nok, men usikkerheten er fortsatt svC&rt stor.
  
  # Den negative binomiske fordelingen (NB) ser ut til C% fungere bedre som modell for
  # den romlige fordelingen av Honningblom enn andre aktuelle fordelinger som er
  # utprC8vd: poisson, hurdeled poisson, zeroinflated poisson, hurdeled NB og 
  # zeroinflated NB.
  ...
  # forekomstarealet for hvert bootstrapsample (og C%r). Tilsvarende, bootstrap
  # analysetidsseriene og estimer tetthet for hvert bootstrapsample (og C%r).
  # For Filletassen bootstrap transektrutetidsseriene og beregn forekomstarealet
  # for hver permutasjon og C%r uten C% endre undersC8kelsesomrC%det i hvert sample. 
  # Bootstrap analysetidsseriene og estimer tetthet for hvert bootstrapsample 
  # (og C%r).
  # For TeneskjC&r bootstrap analysetidsseriene og estimer forekomstarealet og 
  # tettheten for hver permutasjon og C%r.
  # For Skipstadsand bootstrap transektrutetidsseriene, bootstrap 
  # analysetidsseriene og estimer forekomstarealet for hvert C%r fra hvert par av 
  # bootstrapsampler. Benytt bootstrap sample fra analysetidsseriene til ogsC%
  # C% estimere tetthet innenfor forekomstarealet for hvert C%r.
  # I Praksis, slik estimeringen nC% er programmert, kan de samme resampling-
  # rutinene benyttes for alle populasjonene.
  #
  # For hver simulering beregn populasjonsstC8rrelsen for hvert C%r og estimer 
  # konfidens-intervallet fra fordelingen av populasjonsstC8rrelser for hvert C%r.
  # Dog med den begrensning at populasjonsstC8rrelsen er stC8rre eller lik 
...
#Export data
write.csv(df.pop, "path/Herminium_pop_2025.csv")
```
...
setwd("path/Alle lokaliteter 2025.png", width = 840, height = 840)
par(mfrow=c(2,2))
for (loc in localities) {
